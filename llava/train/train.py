...
class ModelArguments:
    ...
    scene_tower: Optional[str] = field(default=None)
        ...
        for sentence in source:
            for DEFAULT_TOKEN in DEFAULT_X_TOKEN.values():
                ...
                if DEFAULT_TOKEN in sentence['value']:
                    ...
                    sentence['value'] = sentence['value'].strip()
                    if "mmtag" in conversation_lib.default_conversation.version:
                        sentence['value'] = sentence['value'].replace(DEFAULT_TOKEN,
                                                                      f'<{X.capitalize()}>' + DEFAULT_TOKEN + f'</{X.capitalize()}>')
...
def preprocess_llama_2(
    ...
    if X is not None:
        input_ids = torch.stack(
            [tokenizer_X_token(prompt, tokenizer, X_TOKEN_INDEX[X], return_tensors='pt') for prompt in conversations],
            dim=0)
    else:
        input_ids = tokenizer(
            conversations,
...
def preprocess_v1(
    ...
    for conversation, target in zip(conversations, targets):
        # print(target, print(conversations))
        total_len = int(target.ne(tokenizer.pad_token_id).sum())
        # print(total_len)
        rounds = conversation.split(conv.sep2)
        # print(rounds)
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if len(parts) != 2:
                break
            parts[0] += sep
            round_len = len(tokenizer_X_token(rou, tokenizer, X_TOKEN_INDEX[X])) + len(
                tokenizer_X_token(conv.sep, tokenizer))
...
def preprocess_plain(
    ...
    # tokenize conversations
    input_ids = [tokenizer_X_token(prompt, tokenizer, X_TOKEN_INDEX[X], return_tensors='pt') for prompt in
                 conversations]
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        print(f'loading train data : {data_path}')
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            if sources[0]['mode'] == 'motion' or sources[0]['mode'] == 'image':
                ...
                path = self.list_data_dict[i]['path']
                video_folder, video_file = path.split('/')
                pose_file = 'frame_{}.npy'.format(int(video_file.split('.')[0]))
                # print(pose_file)
                pose_feature = torch.from_numpy(
                    np.load(os.path.join('dataset/pose_feat/train/{}'.format(video_folder), pose_file)))
                #print(pose_feature.shape,'olg694300pose')
            ...
            if 'image' in sources[0] or sources[0]['mode'] == 'image':
                ...
                if sources[0]['mode'] == 'train':
                    for e in sources:
                        if 'x264' not in sources[0]['path']:
                            e["conversations"][0][
                                "value"] = "Please determine if the people in the video or the video itself show negative emotions.\n<video>"
                        else:
                            e["conversations"][0][
                                "value"] = "Please determine whether the video is an anomalistic video that contains one of Abuse, Arrest, Arson, Assault, Accident, Burglary, Explosion, Fighting, Robbery, Shooting, Stealing, Shoplifting, and Vandalism.\n<video>"
                        if e["conversations"][1]["value"] == "0":
                            if 'x264' not in sources[0]['path']:
                                e["conversations"][1][
                                    "value"] = "The video itself or the people in the video do not show negative emotions."
                            else:
                                e["conversations"][1][
                                    "value"] = "The video is not an anomalistic video."
                        else:
                            if 'x264' not in sources[0]['path']:
                                e["conversations"][1][
                                    "value"] = "The video itself or the people in the video show negative emotions."
                            else:
                                e["conversations"][1][
        ...
        #     else:
        #         return self.__getitem__(random.randint(50000, self.__len__() - 1))
        ...
        for instance in instances:
            for x in DEFAULT_X_TOKEN.keys():
                ...
                if x in instance:
                    ...
                    poses.append(instance['pose_feat'])
                    scenes.append(instance['scene_feat'])
                    keys.append(x)

        batch['images'] = [Xs, poses, scenes, keys]  # we do not change the key's name.
        return batch
...
def train():
    ...
    if is_qwen3_vl:
        ...
        pose_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)

        model.get_model().initialize_scene_modules(
            model_args=model_args,
            fsdp=training_args.fsdp
        )
        scene_tower = model.get_scene_tower()
        scene_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        ...
        moe = model.get_moe()
        moe.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)

        for p in pose_tower.parameters():
            p.requires_grad = True

        for p in model.get_model().pose_projector.parameters():
            p.requires_grad = True

        for p in scene_tower.parameters():
            p.requires_grad = True

        for p in model.get_model().scene_projector.parameters():
            p.requires_grad = True
        ...
        if training_args.bits in [4, 8]:
            ...
            model.get_model().pose_projector.to(dtype=compute_dtype, device=training_args.device)
